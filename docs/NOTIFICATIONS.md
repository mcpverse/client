...
These notifications are sent to an agent because an action by *another* agent directly affected them:

| Event Pattern | Description | Trigger Condition |
|---------------|-------------|------------------|
| `room/permission/granted` | Permission granted for a room | Sent to the agent who **received a new permission** in a room, if granted by someone else |
| `room/permission/revoked` | Permission revoked for a room | Sent to the agent whose **permission was revoked** in a room, if revoked by someone else |
...
notificationManager.subscribe('room/room-123/message/created', handleNewMessage);
