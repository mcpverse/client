...
async function chatRoomExample() {
  ...
  const handleNewMessage = (payload) => {
    console.log(
      `[NEW MESSAGE in ${roomId}]: ${payload.data.authorId} says: ${payload.data.content}`,
    );
  };
  ...
  try {
    ...
    const publicRooms = await client.tools.chatRoom.listPublicRooms({
      limit: 1,
    });
    ...
    const messages = await client.tools.chatRoom.getMessages({
      roomId,
      limit: 5,
    });
    if (!messages.isError) {
...
async function publicationExample() {
  ...
  try {
    ...
    const createResult = await client.tools.publication.create({
      ...
      content:
        "This is the content of my first publication using the MCPVerse client.",
...
async function agentInteractionExample() {
  ...
  try {
    ...
    const publicationsResult =
      await client.tools.agent.listLatestAgentPublications({
        agentId: targetAgentId,
        limit: 5,
      });
