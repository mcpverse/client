...
const AUTH_TEST_CREDENTIALS_FILE_PATH_BASE =
  process.env.AUTH_TEST_CREDENTIALS_FILE_PATH || "./test-credentials"; // Base path
...
if (
  !TEST_MCP_SERVER_URL ||
  !AUTH_TEST_CREDENTIALS_FILE_PATH_BASE ||
  !AUTH_TEST_AGENT_DISPLAY_NAME ||
  !AUTH_TEST_AGENT_BIO ||
  !AUTH_TEST_API_KEY
) {
  throw new Error(
    "Missing required environment variables for testing (TEST_MCP_SERVER_URL, AUTH_TEST_CREDENTIALS_FILE_PATH, AUTH_TEST_AGENT_DISPLAY_NAME, AUTH_TEST_AGENT_BIO, AUTH_TEST_API_KEY). Please check your .env.test setup.",
  );
}
...
describe("MCPVerseClient Registration and Authentication (Real Server)", () => {
  ...
  test("should register a new agent, connect, and save credentials", async () => {
    const registrationStore = new FileCredentialStore(
      registrationCredentialsFilePath,
    );
  ...
  test("should connect and disconnect successfully using existing (just registered) credentials", async () => {
    if (!registeredAgentId) {
      throw new Error(
        "Registration test must run first and set registeredAgentId",
      );
    }
  ...
  test("should fail registration with 401 when API key is invalid", async () => {
    ...
    await expect(clientWithInvalidApiKey.connect()).rejects.toThrow(
  ...
  test("should fail registration with 401 when API key is missing", async () => {
    ...
    await expect(clientWithMissingApiKey.connect()).rejects.toThrow(
      MCPVerseAuthenticationError,
    );
