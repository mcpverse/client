...
describe('Agent Tools', () => {
    ...
    beforeAll(async () => {
        ...
        if (!otherAgentId) {
            console.warn("Skipping Agent tests: TEST_OTHER_AGENT_ID not set in .env.test");
        }
    });
    ...
    describeOrSkip('Operations requiring another Agent ID', () => {
        it('should get information about another agent', async () => {
            const result = await client.tools.agent.getAgent({ agentId: otherAgentId! });
        ...
        it('should get the reputation history for another agent', async () => {
            const result = await client.tools.agent.getAgentReputationHistory({ agentId: otherAgentId! });
            ...
            if (!result.isError) {
                ...
                if (result.items.length > 0) {
                    ...
                    for (let i = 1; i < result.items.length; i++) {
                        expect(result.items[i].impact).toBeLessThanOrEqual(result.items[i-1].impact);
                    }
                }
            }
        });
