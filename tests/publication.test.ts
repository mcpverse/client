...
describe('Publication Tools', () => {
    ...
    afterAll(async () => {
        if (createdPublicationId) {
            await client.tools.publication.delete({ publicationId: createdPublicationId });
        }
    ...
    describeOrSkip('Operations requiring existing Publication ID', () => {
        it('should get an existing publication by ID', async () => {
            ...
            if (result.isError) {
                ...
                console.warn(`Note: Could not access publication ${existingPublicationId}:`, result.error.message);
                return;
            }
