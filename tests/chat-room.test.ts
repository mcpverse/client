...
describe('Chat Room Tools', () => {
    ...
    beforeAll(async () => {
        ...
        if (!otherAgentId || !writeRoomId || !readOnlyRoomId) {
            console.warn(`Skipping some Chat Room tests: Required IDs missing in .env.test ` +
    ...
    describe('Operations on Created Room', () => {
        beforeAll(async () => {
            if (!createdRoomId) throw new Error("Created room ID is null, cannot run tests");
            const getResult = await client.tools.chatRoom.get({ roomId: createdRoomId! });
            if (getResult.isError) {
                throw new Error(`Created room ${createdRoomId} not found or accessible: ${getResult.error.message}`);
            }
        });

        it('should get the details of the created room', async () => {
            const result = await client.tools.chatRoom.get({ roomId: createdRoomId! });
            expect(result.isError).toBe(false);
            if (!result.isError) {
        ...
        it('should get the current agent\'s permission for the created room (should be ADMIN)', async () => {
            const result = await client.tools.chatRoom.getRoomPermission({ roomId: createdRoomId! });
            expect(result.isError).toBe(false);
            if (!result.isError) {
        ...
        it('should list permissions for the created room (should include self)', async () => {
            const result = await client.tools.chatRoom.listPermissions({ roomId: createdRoomId! });
            expect(result.isError).toBe(false);
            if (!result.isError) {
                expect(Array.isArray(result.items)).toBe(true);
                const selfPermission = result.items.find(p => p.agentId === client['credentials']?.agentId);
        ...
        it('should get reputation history for the created room', async () => {
            const result = await client.tools.chatRoom.getReputationHistory({ roomId: createdRoomId! });
            expect(result.isError).toBe(false);
            if (!result.isError) {
        ...
        it('should list latest publications in the created room', async () => {
            const result = await client.tools.chatRoom.listLatestPublications({ roomId: createdRoomId! });
            expect(result.isError).toBe(false);
            if (!result.isError) {
                expect(Array.isArray(result.items)).toBe(true);
            }
    ...
    const describeEnvRoom = () => (writeRoomId && readOnlyRoomId) ? describe : describe.skip;
    describeEnvRoom()('Operations on Existing Rooms (from Env)', () => {
        beforeAll(async () => {
            // Verify the READ_ONLY room's permission level
            const permissionResult = await client.tools.chatRoom.getRoomPermission({ roomId: readOnlyRoomId! });
        ...
        it('should successfully send a message to a room with WRITE access', async () => {
            ...
            const result = await client.tools.chatRoom.sendMessage({ roomId: writeRoomId!, content });
            expect(result.isError).toBe(false);
            if (!result.isError) {
                expect(result.message).toBeDefined();
            }
        ...
        it('should FAIL to send a message to a room with READ_ONLY access', async () => {
            ...
            const response = await client.tools.chatRoom.sendMessage({ roomId: readOnlyRoomId!, content });
            expect(response.isError).toBe(true);
            if (response.isError) {
                expect(response.error.message).toContain('WRITE permission required to send messages to this room.');
            }
        });
        ...
        it('should FAIL to update a room with READ_ONLY access', async () => {
            ...
            expect(response.isError).toBe(true);
            if (response.isError) {
                expect(response.error.message).toContain('ADMIN permission required to update this room.');
            }
        });
    });
}); 